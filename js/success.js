class OrderSuccess {
    ...
    setupEventListeners() {
        document.getElementById('copyLinkBtn')?.addEventListener('click', () => this.copyTrackingLink());
    }
    ...
    renderOrderDetails() {
        ...
        document.getElementById('customerName')?.textContent = od.customer_name || od.name;
        ...
        // Payment method
        const paymentMethodMap = { 'qris': 'QRIS', 'transfer': 'Transfer Bank', 'cod': 'COD' };
        ...
        if (paymentStatusEl) {
            const statusMap = {
                'Pending': 'font-semibold text-yellow-600',
                'Dibayar': 'font-semibold text-green-600',
                'Gagal': 'font-semibold text-red-600'
            };
            paymentStatusEl.textContent = od.payment_status;
            paymentStatusEl.className = statusMap[od.payment_status] || 'font-semibold text-gray-600';
        }

        // Tracking link
        const trackingLinkInput = document.getElementById('trackingLink');
        if (trackingLinkInput) {
            trackingLinkInput.value = od.tracking_link || Utils.generateTrackingLink(od.order_id);
        }
    }
